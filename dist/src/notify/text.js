...
class MessageTemplates {
    ...
    static tokenTransfer(data) {
        const { hash, walletName, walletAddress, received, sent, scanUrl } = data;
    ...
    static bnbTransfer(data) {
        const { hash, walletName, received, sent, scanUrl } = data;
...
<b>已自动添加到监控列表</b>
    ...
    static systemStatus(data) {
        const { monitoredWallets, latestBlock, processedBlocks, foundTransactions, uptime } = data;
        return `
<b>📊 系统状态</b>
...
<b>时间:</b> ${timestamp || new Date().toISOString()}
    ...
    static batchSummary(data) {
        const { blockRange, totalTransactions, bnbTransfers, tokenTransfers, newWallets } = data;
        return `
<b>📈 区块汇总</b>
